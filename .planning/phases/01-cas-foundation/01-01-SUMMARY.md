...
tech-stack:
  ...
  patterns:
    ...
    - Zero-dependency traits crate: slicefs-traits has only thiserror, enabling adapter crates without pulling in implementation deps
...
key-files:
  created:
    ...
    - crates/slicefs-traits/Cargo.toml — traits crate manifest, thiserror only
    - crates/slicefs-traits/src/lib.rs — pub mod declarations + pub use re-exports
    - crates/slicefs-traits/src/error.rs — CasError enum with 6 variants
    - crates/slicefs-traits/src/hash.rs — ChunkHash newtype + ContentHasher trait
    - crates/slicefs-traits/src/chunk.rs — Chunk struct + Chunker trait
    - crates/slicefs-traits/src/block_store.rs — BlockStoreConfig + BlockStore trait
    - crates/slicefs-traits/src/dedup_index.rs — DedupResult enum + DedupIndex trait
    - crates/cas-local/Cargo.toml — stub impl crate, depends on slicefs-traits via path
...
key-decisions:
  ...
  - "slicefs-traits depends only on thiserror — adapter crates can implement traits without cas-local deps"
...
patterns-established:
  - "Pattern 1 (Trait Definition in Zero-Dependency Crate): all trait definitions live in slicefs-traits with no external deps beyond thiserror; every other crate depends on slicefs-traits"
...
**Four sync CAS trait interfaces (ContentHasher, Chunker, BlockStore, DedupIndex) defined in a zero-dependency slicefs-traits crate with ChunkHash newtype, CasError enum, and cas-local skeleton wired for Phase 2/3 stub implementations**
...
1. **Task 1: Initialize workspace and slicefs-traits with all trait definitions** - `41ececf` (feat)
...
- `crates/slicefs-traits/Cargo.toml` — minimal traits crate (thiserror only)
- `crates/slicefs-traits/src/lib.rs` — pub mod + pub use re-exports for ergonomic imports
- `crates/slicefs-traits/src/error.rs` — CasError with NotFound, IntegrityFailure, HashCollision, Io, Chunker, Index
- `crates/slicefs-traits/src/hash.rs` — ChunkHash newtype + ContentHasher trait
- `crates/slicefs-traits/src/chunk.rs` — Chunk struct + Chunker trait
- `crates/slicefs-traits/src/block_store.rs` — BlockStoreConfig (verify_on_read=true default) + BlockStore trait
- `crates/slicefs-traits/src/dedup_index.rs` — DedupResult enum + DedupIndex trait with two-phase bloom design
- `crates/cas-local/Cargo.toml` — depends on slicefs-traits path, blake3, fastbloom, thiserror
- `crates/cas-local/src/lib.rs` — module declarations for 5 stub modules
...
- **slicefs-traits depends only on thiserror** — adapter crates for the owner's algorithms can implement traits without pulling in cas-local's blake3/fastbloom deps
...
- **Verification:** `cargo check -p slicefs-traits` and `cargo check --workspace` both passed after fix
