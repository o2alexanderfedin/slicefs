...
fn test_file_write_at_offset_zero_pads() {
    ...
    let (ino, fh) = fs
        .test_create(1, "sparse.bin", S_IFREG | 0o644, 0o022, 0, 0)
    ...
    assert_eq!(
        content.len(),
        14,
        "file should be 14 bytes: 10 padding + 4 data"
    );
    assert_eq!(
        &content[..10],
        &[0u8; 10],
        "first 10 bytes should be zero-padded"
    );
...
fn test_file_delete_removes_from_directory() {
    ...
    let (ino, fh) = fs
        .test_create(1, "delete_me.txt", S_IFREG | 0o644, 0o022, 0, 0)
        .expect("create delete_me.txt");
    fs.test_release(ino, fh).expect("release");

    // File is accessible before unlink
    fs.meta()
        .lookup(1, "delete_me.txt")
        .expect("should be found before unlink");

    // Unlink removes it
    fs.simulate_unlink(1, "delete_me.txt")
...
fn test_file_create_in_subdirectory() {
    ...
    let (file_ino, fh) = fs
        .test_create(subdir_ino, "nested.txt", S_IFREG | 0o644, 0o022, 0, 0)
...
fn test_file_overwrite_content() {
    ...
    let (ino, fh1) = fs
        .test_create(1, "overwrite.txt", S_IFREG | 0o644, 0o022, 0, 0)
        .expect("create");
    fs.test_write(fh1, 0, b"original data")
        .expect("first write");
    ...
    let (ino2, fh2) = fs
        .test_create(1, "overwrite2.txt", S_IFREG | 0o644, 0o022, 0, 0)
        .expect("create overwrite2");
    fs.test_write(fh2, 0, b"new content")
...
fn test_dir_mkdir_has_dot_entries() {
    ...
    let dir_ino = fs
        .simulate_mkdir(1, "newdir", S_IFDIR | 0o755, 0o022, 0, 0)
        .expect("mkdir newdir");
...
fn test_dir_nested_mkdir() {
    ...
    let parent_ino = fs
        .simulate_mkdir(1, "parent", S_IFDIR | 0o755, 0o022, 0, 0)
        .expect("mkdir parent");
    let child_ino = fs
        .simulate_mkdir(parent_ino, "child", S_IFDIR | 0o755, 0o022, 0, 0)
        .expect("mkdir parent/child");

    let found = fs
        .meta()
        .lookup(parent_ino, "child")
        .expect("lookup parent/child");
...
fn test_dir_rmdir_empty_succeeds() {
    ...
    fs.simulate_mkdir(1, "removeme", S_IFDIR | 0o755, 0o022, 0, 0)
        .expect("mkdir");
    fs.simulate_rmdir(1, "removeme")
        .expect("rmdir should succeed on empty dir");
...
fn test_dir_rmdir_nonempty_returns_enotempty() {
    ...
    let dir_ino = fs
        .simulate_mkdir(1, "nonempty", S_IFDIR | 0o755, 0o022, 0, 0)
        .expect("mkdir");
    let (fino, fh) = fs
        .test_create(dir_ino, "file.txt", S_IFREG | 0o644, 0o022, 0, 0)
...
fn test_dir_rmdir_on_file_returns_enotdir() {
    ...
    let (ino, fh) = fs
        .test_create(1, "notadir.txt", S_IFREG | 0o644, 0o022, 0, 0)
        .expect("create file");
    fs.test_release(ino, fh).expect("release");
...
fn test_dir_rmdir_decrements_parent_nlinks() {
    ...
    fs.simulate_mkdir(1, "tmpdir", S_IFDIR | 0o755, 0o022, 0, 0)
        .expect("mkdir");
...
fn test_rename_within_same_directory() {
    ...
    let (ino, fh) = fs
        .test_create(1, "old.txt", S_IFREG | 0o644, 0o022, 0, 0)
        .expect("create old.txt");
    fs.test_write(fh, 0, b"rename test").expect("write");
    fs.test_release(ino, fh).expect("release");

    fs.simulate_rename(1, "old.txt", 1, "new.txt", 0)
        .expect("rename");

    let found_ino = fs
        .meta()
        .lookup(1, "new.txt")
        .expect("new name should be accessible");
...
fn test_rename_across_directories() {
    ...
    let dir_a = fs
        .simulate_mkdir(1, "dir_a", S_IFDIR | 0o755, 0o022, 0, 0)
        .expect("mkdir dir_a");
    let dir_b = fs
        .simulate_mkdir(1, "dir_b", S_IFDIR | 0o755, 0o022, 0, 0)
        .expect("mkdir dir_b");

    let (ino, fh) = fs
        .test_create(dir_a, "file.txt", S_IFREG | 0o644, 0o022, 0, 0)
        .expect("create in dir_a");
    fs.test_write(fh, 0, b"cross-dir rename").expect("write");
    fs.test_release(ino, fh).expect("release");

    fs.simulate_rename(dir_a, "file.txt", dir_b, "moved.txt", 0)
        .expect("cross-dir rename");

    let found = fs
        .meta()
        .lookup(dir_b, "moved.txt")
        .expect("file should be in dir_b");
...
fn test_rename_overwrites_existing_target() {
    ...
    let (src_ino, fh1) = fs
        .test_create(1, "src.txt", S_IFREG | 0o644, 0o022, 0, 0)
        .expect("create src");
    fs.test_write(fh1, 0, b"source content").expect("write src");
    fs.test_release(src_ino, fh1).expect("release src");

    let (dst_ino, fh2) = fs
        .test_create(1, "dst.txt", S_IFREG | 0o644, 0o022, 0, 0)
        .expect("create dst");
    fs.test_write(fh2, 0, b"destination content")
        .expect("write dst");
    ...
    let found = fs
        .meta()
        .lookup(1, "dst.txt")
        .expect("dst.txt still accessible");
    ...
    assert!(
        dst_inode_result.is_err(),
        "overwritten dst inode should be deleted"
    );
}
...
fn test_rename_directory() {
    ...
    let dir_ino = fs
        .simulate_mkdir(1, "olddir", S_IFDIR | 0o755, 0o022, 0, 0)
        .expect("mkdir");

    fs.simulate_rename(1, "olddir", 1, "newdir", 0)
        .expect("rename dir");
...
fn test_rename_editor_pattern() {
    ...
    let (orig_ino, fh1) = fs
        .test_create(1, "document.txt", S_IFREG | 0o644, 0o022, 0, 0)
        .expect("create document.txt");
    fs.test_write(fh1, 0, b"original content")
        .expect("write original");
    fs.test_release(orig_ino, fh1).expect("release original");

    // Create temp file with new content
    let (tmp_ino, fh2) = fs
        .test_create(1, "document.txt.tmp", S_IFREG | 0o644, 0o022, 0, 0)
        .expect("create temp");
    fs.test_write(fh2, 0, b"updated content")
        .expect("write updated");
    fs.test_release(tmp_ino, fh2).expect("release temp");

    // Atomic rename temp over original (editor save pattern)
    fs.simulate_rename(1, "document.txt.tmp", 1, "document.txt", 0)
        .expect("atomic rename");

    let found_ino = fs
        .meta()
        .lookup(1, "document.txt")
        .expect("document.txt should exist");
    let content = read_content(&fs, found_ino);
    assert_eq!(
        content, b"updated content",
        "document.txt should have updated content after rename"
    );
...
fn test_symlink_readlink_returns_target() {
    ...
    let sym_ino = fs
        .simulate_symlink(1, "link.txt", "/target/path", 0, 0)
        .expect("create symlink");

    let target = fs.simulate_readlink(sym_ino).expect("readlink");
    assert_eq!(
        target, "/target/path",
        "readlink should return the original target"
    );
}
...
fn test_symlink_dangling_allowed() {
    ...
    let sym_ino = fs
        .simulate_symlink(1, "dangling", "/nonexistent/path/to/nowhere", 0, 0)
        .expect("dangling symlink creation should succeed");

    let target = fs
        .simulate_readlink(sym_ino)
        .expect("readlink dangling symlink");
...
fn test_symlink_size_equals_target_length() {
    ...
    let sym_ino = fs
        .simulate_symlink(1, "libfoo.so", target, 0, 0)
        .expect("create symlink");
...
fn test_symlink_and_file_are_distinct_inodes() {
    ...
    let sym_ino = fs
        .simulate_symlink(1, "link.txt", "/target", 0, 0)
        .expect("create symlink");

    let (file_ino, fh) = fs
        .test_create(1, "file.txt", S_IFREG | 0o644, 0o022, 0, 0)
        .expect("create regular file");
    fs.test_release(file_ino, fh).expect("release");

    assert_ne!(
        sym_ino, file_ino,
        "symlink and file should have different inodes"
    );

    let sym_meta = fs.meta().get_inode(sym_ino).unwrap();
    // Symlink mode has S_IFLNK set
    assert!(
        sym_meta.mode & 0o170_000 == 0o120_000,
        "symlink should have S_IFLNK type bits"
    );
}
...
fn test_link_nlinks_equals_two() {
    ...
    let (ino, fh) = fs
        .test_create(1, "original.txt", S_IFREG | 0o644, 0o022, 0, 0)
    ...
    assert_eq!(
        meta_after.nlinks, 2,
        "nlinks should be 2 after hard link creation"
    );
}
...
fn test_link_unlink_one_keeps_inode() {
    ...
    fs.simulate_link(ino, 1, "link.txt")
        .expect("create hard link");

    // Remove one hard link
    fs.simulate_unlink(1, "original.txt")
    ...
    let found_ino = fs
        .meta()
        .lookup(1, "link.txt")
        .expect("hard link still accessible");
...
fn test_link_cross_directory() {
    ...
    fs.simulate_link(ino, dir_ino, "cross_link.txt")
    ...
    let found = fs
        .meta()
        .lookup(dir_ino, "cross_link.txt")
        .expect("link in subdir");
...
fn test_link_to_directory_returns_eperm() {
    ...
    let dir_ino = fs
        .simulate_mkdir(1, "adir", S_IFDIR | 0o755, 0o022, 0, 0)
        .expect("mkdir");
...
fn test_truncate_to_shorter_length() {
    ...
    let (ino, fh) = fs
        .test_create(1, "truncate.txt", S_IFREG | 0o644, 0o022, 0, 0)
        .expect("create");
    ...
    assert_eq!(
        content, b"hello",
        "content should be 'hello' after truncate to 5"
    );
}
...
fn test_truncate_to_zero() {
    ...
    let (ino, fh) = fs
        .test_create(1, "zeroed.txt", S_IFREG | 0o644, 0o022, 0, 0)
        .expect("create");
    fs.test_write(fh, 0, b"some content to erase")
        .expect("write");
    fs.test_release(ino, fh).expect("release");
...
fn test_chmod_changes_mode_bits() {
    ...
    fs.test_setattr_mode(ino, S_IFREG | 0o755)
        .expect("chmod 755");
    ...
    assert_eq!(
        after.mode & 0o7777,
        0o755,
        "mode bits should be 755 after chmod"
    );
}
...
fn test_chown_changes_uid_gid() {
    ...
    let (ino, fh) = fs
        .test_create(1, "own.txt", S_IFREG | 0o644, 0o022, 1000, 1000)
        .expect("create");
    fs.test_release(ino, fh).expect("release");

    fs.test_setattr_uid_gid(ino, Some(2000), Some(2000))
        .expect("chown");
...
fn test_setattr_mtime_explicit() {
    ...
    let (ino, fh) = fs
        .test_create(1, "ts.txt", S_IFREG | 0o644, 0o022, 0, 0)
        .expect("create");
    fs.test_release(ino, fh).expect("release");

    fs.test_setattr_mtime(ino, 1_700_000_000, 0)
        .expect("set mtime");

    let meta = fs.meta().get_inode(ino).unwrap();
    assert_eq!(
        meta.mtime_sec, 1_700_000_000,
        "mtime_sec should be explicitly set"
    );
}
...
fn test_dedup_identical_content_same_digest() {
    ...
    let (ino1, fh1) = fs
        .test_create(1, "dup1.txt", S_IFREG | 0o644, 0o022, 0, 0)
        .expect("create dup1");
    fs.test_write(fh1, 0, content).expect("write dup1");
    fs.test_release(ino1, fh1).expect("release dup1");

    let (ino2, fh2) = fs
        .test_create(1, "dup2.txt", S_IFREG | 0o644, 0o022, 0, 0)
        .expect("create dup2");
    fs.test_write(fh2, 0, content)
        .expect("write dup2 (identical)");
...
fn test_dedup_refcount_incremented_for_identical_content() {
    ...
    let (ino1, fh1) = fs
        .test_create(1, "rc1.txt", S_IFREG | 0o644, 0o022, 0, 0)
        .expect("create rc1");
    ...
    let (ino2, fh2) = fs
        .test_create(1, "rc2.txt", S_IFREG | 0o644, 0o022, 0, 0)
        .expect("create rc2");
    ...
    assert_eq!(
        rc_after_second, 2,
        "refcount should be 2 after two identical files"
    );
}
...
fn test_dedup_refcount_decremented_on_overwrite() {
    ...
    let (ino, fh) = fs
        .test_create(1, "overwrite_rc.txt", S_IFREG | 0o644, 0o022, 0, 0)
        .expect("create");
    fs.test_write(fh, 0, b"original content")
        .expect("write original");
...
fn test_dedup_logical_bytes_counts_both_files() {
    ...
    let (ino1, fh1) = fs
        .test_create(1, "lb1.txt", S_IFREG | 0o644, 0o022, 0, 0)
        .expect("create lb1");
    ...
    let (ino2, fh2) = fs
        .test_create(1, "lb2.txt", S_IFREG | 0o644, 0o022, 0, 0)
        .expect("create lb2");
